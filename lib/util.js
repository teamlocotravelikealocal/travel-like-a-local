...
exports.getImageLinks = function(req, res) {
  ...
  let url = req.query.link || `https://freeradikal.smugmug.com/`;
  let options = {
    url: `https://api.diffbot.com/v3/image?token=${DIFFBOT_API_KEY}&url=${url}`,
    json:true,
    headers: {
      'User-Agent': 'request',
  ...
  request(options, function(err, response, body) {
    console.log('inside getImageLinks, link is...', url);
    if(err){
      ...
      console.log('SUCCESS: response in diffbot Request...', url);
      res.send(url);
      // res.send('https://photos.smugmug.com/Travel/Italy-Venizia/i-fhbXXKs/0/921a50aa/M/IMG_0992_3_4_tonemapped-M.jpg');
