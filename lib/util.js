...
exports.getSuggestedFriendsForUser = function(req, res){
  ...
  // console.log('getSuggestedFriendsForUser...loggedUser is...', loggedUser);
  if ( loggedUser === 'not logged in' ) {
    res.send([]);
  } else {
    connection.query(queryStr, function(error, result){
      if (error){
        ...
        // console.log('getSuggestedFriendsForUser.....result...', result);
        res.send(result.filter( (friend) => {
...
exports.getFriendListForUser = function(req, res){
  ...
  // console.log('inside getFriendList db query');
  connection.query(queryStr, function(error, result){
    if (error){
      console.log(error.sqlMessage);
      res.send(error.sqlMessage);
    } else {
      // console.log('inside getFriendList result is...', result);
      res.send(result);
    }
  });
};
...
exports.getEventsFromEventbrite = function(req, res) {
  ...
  let url = `https://www.eventbriteapi.com/v3/events/search/?token=${EVENTBRITE_OAUTH_TOKEN}&q=music&location.within=20mi&location.latitude=${lattitude}&location.longitude=${longitude}`;
  // let url = `https://www.eventbriteapi.com/v3/events/search/?&q=music&location.within=20mi&location.latitude=${lattitude}&location.longitude=${longitude}`;
  let token = "Bearer " + EVENTBRITE_OAUTH_TOKEN;
  let options = {
    url: url,
    json:true,
    headers: {
  ...
  // let url = `https://www.eventbriteapi.com/v3/events/search/?token=${EVENTBRITE_OAUTH_TOKEN}&q=music&location.within=20mi&location.latitude=${lattitude}&location.longitude=${longitude}`;
  // let url = `https://www.eventbriteapi.com/v3/events/search/?&q=music&location.within=20mi&location.latitude=${lattitude}&location.longitude=${longitude}`;
  // console.log("Options for https:...", options);
  https.get(url, (resp) => {
    let data = '';
    let eventNames = [];
    resp.on('data', (chunk) => {
    ...
    // The whole response has been received. Print out the result.
    resp.on('end', () => {
      console.log('eventbrite response data is...');
...
exports.createSession = function(req, res, newUser) {
  return req.session.regenerate(function() {
      ...
      res.send({isLogged: true, user: newUser});
      ...
      if (results.length > 0) {
        bcrypt.compare(password, results[0].password, function(err, result){
          if (err) {
            ...
            if (result) {
              // console.log('username inside handleLogin',username)
...
exports.handleSignup = function(req, res) {
  var username = req.body.credentials.userName;
  var password = req.body.credentials.password;
  console.log('in handleSignup....req.body...',req.body);
  console.log('in handleSignup....username/password...', username + '/' + password);
  ...
  connection.query('insert into users (username, password) values ('+ JSON.stringify(username) +','+ JSON.stringify(hash) +')', function(err, results){
    if (err) {
      console.log('Error in signing up....', err.sqlMessage);
      res.redirect('signup.html');
    } else {
      exports.createSession(req, res, username);
    }
