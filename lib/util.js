...
exports.getImageLinks = function(req, res) {
  ...
  let options = {
    url: `https://api.diffbot.com/v3/image?token=${DIFFBOT_API_KEY}&url=https://freeradikal.smugmug.com/`,
    json:true,
    headers: {
      'User-Agent': 'request',
  ...
  console.log('inside getImageLinks');
  request(options, function(err, response, body) {
    if(err){
      console.log('Error in utils.getImageLinks ', err.message);
    } else {
      console.log('diffbot Request SUCCESS');
      // console.log('response in diffbot Request...',response);
      var results = body.objects;
      let url = [];
      url.push(results[0].url);
      console.log('response in diffbot Request...', results);
