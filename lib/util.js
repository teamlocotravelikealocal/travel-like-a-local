...
const EVENTBRITE_API_KEY = require('./../db/config.js').EVENTBRITE_API_KEY;
...
exports.getEventsFromEventbrite = function(req, res) {
  ...
  console.log('request inside utils.getEventsFromEventbrite...');
  ...
  let options = {
    ...
    headers: {
      'User-Agent': 'request',
      "Content-Type": "application/json",
      'Authorization': 'Bearer ' + 'YN5YFP4JMMCBE5AYYGJ2',
      "Access-Control-Allow-Origin": true
    }
  };
  console.log('successful googleMapsClient call for events...');
  ...
  //       console.log('ERROR: in utils.getEventsFromGoogleMapsAndEventbrite ', err.message);
  //     } else {
  //       let results = body.events;
  //       let eventNames = [];
  //       if ( results && results.length > 0 ) {
  //         results.map(item => 
  //         eventNames.push(item.name.text));
  //       }
  //       console.log('SUCCESS: response in eventbrite api Request...');
  //       res.send(eventNames);

  //     }
  //   });

  https.get(`https://www.eventbriteapi.com/v3/events/search/?token=${EVENTBRITE_API_KEY}&q=music&location.within=20mi&location.latitude=${lattitude}&location.longitude=${longitude}`, (resp) => {
    let data = '';
    let eventNames = [];
    resp.on('data', (chunk) => {
      data += chunk;
    ...
    resp.on('end', () => {
      ...
      if (results.length > 0 ) {
        results.map(event => eventNames.push(JSON.stringify({
